    ...
    <div className="w-full bg-surface flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-4 py-3.5">
        <a href="/" className="flex items-center group">
          ...
          <span className="text-md font-semibold text-white/80 group-hover:text-white transition-colors">DocsChat</span>
        </a>
        {onCloseMobile && (
          <button
            onClick={onCloseMobile}
            className="lg:hidden text-white/40 hover:text-white/70 transition-colors cursor-pointer"
          >
      ...
      <div className="flex-1 overflow-y-auto px-1.5 pb-2 min-h-0">
        {loading ? (
          ...
          <div className="flex flex-col items-center pt-16 px-4">
            ...
            <p className="text-xs text-white/20 text-center leading-relaxed max-w-[180px] mb-6">
          ...
          <div className="flex flex-col gap-0.5">
            ...
            {sources.map(source => (
              <div key={source.id} className="group flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white/[0.04] transition-all cursor-default">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${source.status === 'error'
                  ? 'bg-red-500/10'
      ...
      <div className="border-t border-white/[0.06] px-3 py-2.5">
        {user?.email ? (
          ...
          <div className="flex items-center gap-2 px-2 py-1.5">
            ...
            <span className="text-[11px] text-white/15">Only PDF files are supported</span>
          </div>
        )}
      </div>
